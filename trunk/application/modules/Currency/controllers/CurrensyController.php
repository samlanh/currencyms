...
class Currency_CurrensyController extends Zend_Controller_Action
{
    ...
    public function currenAction()
    {
       
		
    	if($this->getRequest()->isPost()){
			$agentdata=$this->getRequest()->getPost();	
			$Currency= new Currency_Model_DbTable_table();				
			try {
				$db = $Currency->addStudent($agentdata);				
				//Application_Form_FrmMessage::Sucessfull('ការ​បញ្ចូល​​ជោគ​ជ័យ', self::REDIRECT_URL);		
			} catch (Exception $e) {
				$this->view->msg = 'ការ​បញ្ចូល​មិន​ជោគ​ជ័យ';
			}
		}
    	$pructis=new Currency_Form_FrmCurrency();
    	$frm = $pructis->addCurrency();
