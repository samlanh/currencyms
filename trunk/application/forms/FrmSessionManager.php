...
class Application_Form_FrmSessionManager extends Zend_Form
{
	...
	static function clearSessionSearch(){
		...
		if($module == 'kbank'){
			...
			$search_kbank->unsetAll();
			
			$session_withdraw=new Zend_Session_Namespace('search_withdraw');
			$session_withdraw->unsetAll();
		}
		if($module == 'capital'){
			//Cleat session in module exchange
			$search_kbank=new Zend_Session_Namespace('search_capital');
			$search_kbank->unsetAll();
			if($controller=='stock'){
				$search_stock=new Zend_Session_Namespace('search_stock');
				$search_stock->unsetAll();
			}
		}	
