...
class Application_Model_DbTable_DbUsers extends Zend_Db_Table_Abstract
{
	...
	{	
		$db = $this->getAdapter();
		$sql = "SELECT count(*) AS max_user FROM `cs_users`";	
		$row = $db->fetchRow($sql);	
	...
	public function getArrAcl($user_type_id){
		$db = $this->getAdapter();
		$sql = "SELECT aa.module, aa.controller, aa.action FROM rsv_acl_user_access AS ua  INNER JOIN rsv_acl_acl AS aa ON (ua.acl_id=aa.acl_id) WHERE ua.user_type_id='".$user_type_id."'";
